...
test("registers a disconnect handler bound to the current room and slug", async () => {
  await renderHook(() =>
    useRegisterActiveRoomDisconnect("quiet-tiger-42", jest.fn()),
  )
...
test("forwards the forced-disconnect callback to the registry", async () => {
  ...
  await renderHook(() =>
    useRegisterActiveRoomDisconnect("quiet-tiger-42", onForcedDisconnect),
  )
