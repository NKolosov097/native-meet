...
afterEach(() => {
  const sentinel = registration("sentinel")
...
test("disconnects the active room when the link targets another room", async () => {
  ...
  await disconnectActiveRoom("room-b")
...
test("keeps the call alive when the link targets the active room", async () => {
  const roomA = registration("room-a")
  registerActiveRoom(roomA)

  await disconnectActiveRoom("room-a")
...
test("releases the slot so the same handler is never disconnected twice", async () => {
  ...
  await disconnectActiveRoom("room-b")
  await disconnectActiveRoom("room-c")
